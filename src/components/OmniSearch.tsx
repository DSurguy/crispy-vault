...
export default function OmniSearch({ className }: OmniSearchProps) {
  ...
  const editableClassName = twMerge("grow mx-2 outline-none", currentInput.length ? '' : placeholderClassName);

  useEffect(() => {
    if( currentInput ) {
      // TODO: parse to see if we have a command

      if( currentInput.startsWith("tag:") ){
  ...
  return <div className={mergedOuterClassName}>
    <div className="flex">{tags.map(tag => <div key={tag}>{tag}</div>)}</div>
    <div
